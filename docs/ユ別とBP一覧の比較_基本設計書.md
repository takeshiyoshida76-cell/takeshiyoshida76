...
### 1.2 背景
ユ別スプレッドシートに記載された要員情報が、BP情報一覧に正確に登録されていない場合、業務管理に支障をきたす可能性がある。本システムは、未登録の要員を特定し、管理者に通知することで、データ整合性の向上と業務効率化を図る。週次実行により、より頻繁なチェックを実現し、対象月を実行日の日付に基づいて動的に決定（9日以前: 前々月、10日以降: 前月）する。
...
- **処理内容**:
  1. 実行日の日付に基づいて対象月を決定（9日以前: 前々月、10日以降: 前月）。
  2. 管理者マスタから顧客名＋案件名および顧客名のみをキーとする管理者マップを作成。
  3. 対象月のユ別ファイルを取得し、各ファイルの名前をBP情報一覧と照合。
  4. 未登録の名前を管理者および顧客・案件単位でグループ化。
  5. 結果をログ出力およびGoogle Chatに通知。
...
### 4.1 ユ別スプレッドシート
- **ファイル名**: `{年}.{月}_[部門名]`（例: `2025.09_デジタル推進部`）。対象月は実行日の日付に基づいて動的に決定（9日以前: 前々月、10日以降: 前月）。
- **シート名**: 売上・支払情報
- **列構成**:
...
```mermaid
graph TD
    A[開始] --> A1[実行日の日付に基づいて対象月を決定（9日以前: 前々月、10日以降: 前月）]
    A1 --> B[管理者マスタを読み込む]
    B --> C{シート存在確認}
