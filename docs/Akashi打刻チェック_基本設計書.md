...
3. **リトライ処理:** ログイン失敗時（例：ネットワークエラー、セッション確立失敗）、最大3回、1～2秒間隔でリトライします。// 修正: リトライ処理を追加
4. **セッション確立:** リダイレクトヘッダーから**最終セッションCookie**を取得し、以降のデータ取得リクエストで使用します。
5. **リトライアウト時の通知:** リトライがすべて失敗した場合、Google Chatに「勤怠チェック失敗」メッセージを送信（メンションなし）。// 修正: エラー通知を追加
