...
| **予定退勤** (`scheduledEnd`) | `<td>` (Index 4) **または** `<td>` (Index 11)| **Akashiの仕様により、Index 4の値は残業申請されても変更されない。 Index 11 (理由欄)** に `残業終了=HH:MM` のキーワードがある場合、その `HH:MM` の値で **Index 4 の値を上書きし** 、チェックに使用する。キーワードがない場合は Index 4 の値をそのまま使用。 |
