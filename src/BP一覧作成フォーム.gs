...
function onSendChat(e) {
  ...
  const WEBHOOK_URL = PropertiesService.getScriptProperties().getProperty('CHAT_OUTER_WEBHOOKURL');
  ...
  message += "BP情報一覧のURL： " + PropertiesService.getScriptProperties().getProperty('BPICHIRAN_URL');
...
function addEditUrlsToExistingResponses() {
  ...
  const spreadsheet = SpreadsheetApp.openById(PropertiesService.getScriptProperties().getProperty('BPICHIRAN_FILE_ID'));
...
function postMessageToChat() {
  ...
  message += "BP情報一覧：" & PropertiesService.getScriptProperties().getProperty('BPICHIRAN_URL') & "\n"
  message += "●新規登録URL \n"
  message += "新規入力フォーム：" & PropertiesService.getScriptProperties().getProperty('BP_FORM_URL') & "\n"
